...
var Module = {
	...
	UE4_indexedDBVersion: 202212180923, // Bump this number to invalidate existing IDB storages in browsers.
};
...
$(document).ready(function() {
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('https://dl.dropboxusercontent.com /s/c3bp6fugz90s2ve/MyProject-HTML5-Shipping.wasm?dl=0'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('MyProject-HTML5-Shipping.js')).then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/pete8bit/tilda-test/96843780bf6d80bd7bbc6cb81fef6a4fc572b37f/MyProject-HTML5-Shipping.data.js'));
		var utilityJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/pete8bit/tilda-test/d96092484a5c1cd67dc2941a6e71809179d6ea5a/Utility.js')).then(addScriptToDom);
		var dataDownload =
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('https://dl.dropboxusercontent.com/s/56c5a19mx316q4w/MyProject-HTML5-Shipping.data?dl=0'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('https://dl.dropboxusercontent.com/s/56c5a19mx316q4w/MyProject-HTML5-Shipping.data?dl=0')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
